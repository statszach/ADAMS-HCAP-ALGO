...
diag_vars <- c(
    "diagnosis_adams", "diagnosis_adjusted", "diagnosis_adjusted2",
...
fulldata <- as.data.table(Reduce(
    function(x, y) left_join(x, y, by = "ADAMSSID"),
    list(
        tidied, algo_data[, .(ADAMSSID, diagnosis_true = diagnosis_adjusted_2cat, predicted_hcap = predicted_2cat)],
...
sens_hcap <- fulldata %>%
    yardstick::sens(truth = diagnosis_true, estimate = predicted_hcap, case_weights = weight) %>%
    pull(.estimate)
spec_hcap <- fulldata %>%
    yardstick::spec(truth = diagnosis_true, estimate = predicted_hcap, case_weights = weight) %>%
    pull(.estimate)
acc_hcap <- fulldata %>%
    yardstick::accuracy(truth = diagnosis_true, estimate = predicted_hcap, case_weights = weight) %>%
    pull(.estimate)
...
wtable_hcap <- survey::svytable(as.formula("~dementia_hcap_num + dementia_true_num"), design = fulldata_design)
kappa_hcap <- vcd::Kappa(wtable_hcap, weights = "Fleiss-Cohen")$Unweighted[[1]]
...
sens_1066 <- fulldata %>%
    yardstick::sens(truth = diagnosis_true, estimate = predicted_1066, case_weights = weight, na_rm = TRUE) %>%
    pull(.estimate)
spec_1066 <- fulldata %>%
    yardstick::spec(truth = diagnosis_true, estimate = predicted_1066, case_weights = weight, na_rm = TRUE) %>%
    pull(.estimate)
acc_1066 <- fulldata %>%
    yardstick::accuracy(truth = diagnosis_true, estimate = predicted_1066, case_weights = weight, na_rm = TRUE) %>%
    pull(.estimate)
auc_1066 <- fulldata %>%
    yardstick::roc_auc(truth = diagnosis_true, prob1066, case_weights = weight, na_rm = TRUE) %>%
    pull(.estimate)
...
wtable_1066 <- survey::svytable(as.formula("~dementia_1066_num + dementia_true_num"), design = fulldata_design)
kappa_1066 <- vcd::Kappa(wtable_1066, weights = "Fleiss-Cohen")$Unweighted[[1]]
...
summary_table <- data.table(
    ...
    weighted_kappa = c(NA, kappa_hcap, kappa_1066),
    auc = c(NA, NA, auc_1066)
)

## all columns to two decimal places where relevant
round_cols <- c(
    "true_positives_weighted", "true_negatives_weighted", "false_positives_weighted", "false_negatives_weighted", "weighted_prevalence",
    "weighted_sensitivity", "weighted_specificity", "weighted_accuracy", "weighted_kappa", "auc"
)
...
setnames(
    summary_table, c(
        "category", "n_dementia", "total_n", "weighted_prevalence", "true_positives_weighted", "true_negatives_weighted",
        "false_positives_weighted", "false_negatives_weighted", "weighted_sensitivity", "weighted_specificity",
        "weighted_accuracy", "weighted_kappa", "auc"
    ),
    c(
        "Category", "N with Dementia", "Total N", "Prevalence*", "TP*", "TN*", "FP*", "FN*",
        "Sensitivity*", "Specificity*", "Accuracy*", "Kappa*", "AUC*"
    )
)
...
analysis_combos_3cat <- as.data.table(expand.grid(
    truths = c("diagnosis_adjusted", "diagnosis_adams", "diagnosis_adjusted2"),
...
make_contingency_plot <- function(algorithm_data = algo_data, row) {
    truth <- version_map[, truths][row]
    algo <- version_map[, algorithms][row]
    ...
    kappas <- vcd::Kappa(weighted_table, weights = "Fleiss-Cohen")
    kappas_confint <- confint(kappas)

    ## get text
    text_toinclude <- paste0(
        "Unweighted Kappa = ", sprintf("%.2f", kappas$Unweighted[1]),
    ...
    plot <- ggplot(table_dt, aes(x = truth, y = algo, fill = freq)) +
        ...
        theme_void() +
        theme(
            axis.text = element_text(size = 8), axis.title = element_text(size = 8, face = "bold", margin = margin(b = 5)),
...
disagreement_dt[, combo := factor(paste0(diagnosis_adjusted, " - ", predicted_3cat),
    levels = c(
        "Normal - Normal", "Normal - MCI", "Normal - Dementia",
...
disagreement_diagnosis_dt <- disagreement_dt[, .(
    prop_diagnosis = sum(weight) / unique(total_N_combo),
    true_diagnosis = unique(diagnosis_adjusted),
    N_label = unique(N_cat)
), by = .(diagnosis, combo)]
...
agreement_diagnosis_dt <- agreement_dt[, .(
    prop_diagnosis = sum(weight) / unique(total_N_agreement),
    true_diagnosis = unique(diagnosis_adjusted),
    combo = unique(combo), N_label = unique(N_cat)
), by = diagnosis]

## put together
full_diagnosis_dt <- rbind(disagreement_diagnosis_dt, agreement_diagnosis_dt)
full_diagnosis_dt[, diagnosis := factor(diagnosis, levels = rev(c(
    "Normal", "Psychiatric and alcohol",
...
disagreement_plot <- ggplot(full_diagnosis_dt, aes(x = combo, y = prop_diagnosis, fill = diagnosis)) +
    ...
    scale_fill_manual(
        values = c("#2E598C", "#9BD1F2", "#f75e3b", "#F7AF99", "#2b5627", "#59a852", "#B4CF66"),
...
algo_data[, educ := factor(
    fcase(
        degree == "Less than high school or GED", "Less than high school or GED",
...
kappas_3cat <- rbindlist(lapply(
    c("age_group", "gender", "educ", "race"),
    function(x) {
        get_kappa_bycat(
...
kappas_3cat[, category := factor(category, levels = c(
    "70-79", "80-89", "90+", "Men", "Women",
...
get_riskratios <- function(predictor) {
    ...
    m_params_dt <- data.table(
        cat = m_params$Parameter[m_select], rr = m_params$Coefficient[m_select],
        rr_low = m_params$CI_low[m_select], rr_high = m_params$CI_high[m_select]
    )
    m_dt <- rbind(
        data.table(cat = paste0(predictor, algo_data[, levels(get(predictor))][1]), rr = 0, rr_low = NA, rr_high = NA),
        m_params_dt
    )
    ...
    u_params_dt <- data.table(
        cat = u_params$Parameter[u_select], rr = u_params$Coefficient[u_select],
    ...
    o_params_dt <- data.table(
        cat = o_params$Parameter[o_select], rr = o_params$Coefficient[o_select],
...
## plot results
plot_breaks <- c(0.2, 0.4, 0.6, 0.8, 1, 2, 3, 4)
log_breaks <- log(plot_breaks)
...
rr_plot <- ggplot(rr_plot_dt, aes(x = cat_label, y = rr, ymin = rr_low, ymax = rr_high)) +
    ...
    geom_segment(
        data = rr_plot_dt[rr_low < y_limits[1]], aes(x = cat_label, xend = cat_label, y = upper_cap, yend = lower_cap),
        arrow = arrow(length = unit(0.3, "cm"), type = "closed"), show.legend = FALSE, color = "#2d6a2d"
    ) +
    geom_segment(
        data = rr_plot_dt[rr_high > y_limits[2]], aes(x = cat_label, xend = cat_label, y = lower_cap, yend = upper_cap),
        arrow = arrow(length = unit(0.3, "cm"), type = "closed"), show.legend = FALSE, color = "#2d6a2d"
    ) +
